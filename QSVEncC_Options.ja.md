...
手書きGLSLシェーダによる輝度エンハンス / 2倍アップスケールチェーンを有効にする。  
7つのモードはbloc97 Anime4K (MIT)ベース。antiring・chroma_resize=jointはJ. Chrisostomo (MIT)のシェーダ。  
CNNモデルは使用せず、手動チューニングされたOpenCLカーネルで動作する（CNN系モデルは --vpp-onnx を使用）。
...
登録済みONNXモデルのmodels.jsonが格納されたディレクトリを指定する。
