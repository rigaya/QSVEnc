...
- **Parameters**
  ...
  - colormatrix=&lt;string&gt; (デフォルト: auto)  
    色行列。[`--colormatrix`](#--colormatrix-string) と同じ名前を受け付ける。`--vpp-onnx` で対応するのは auto / auto_res / bt601 / smpte170m / bt470bg / bt709 / bt2020 / bt2020nc。
  - colormatrix_out=&lt;string&gt; (デフォルト: auto)
    出力側 RGB→YUV 変換の色行列。`colormatrix` と同じ名前を受け付ける。auto では `colormatrix` と同じ色行列を使用する。BT.2020/PQ RGB を出力する SDR→HDR モデルでは bt2020 を指定する。
...
- **使用可能なモデル名一覧**
  ...
  登録モデルの models.json に `"colormatrix_out": "bt2020"` があり、`colormatrix_out=auto` の場合は、QSVEnc が登録された出力側色行列を自動的に使用する。
