...
void copy_yuy2(void *frame, BYTE *dst_Y, BYTE *dst_C, const int width, const int height, const int pitch) {
	...
	for (y = 0; y < height; y++) {
		...
		for (; p < p_fin; p += 64, Y += 64) {
			...
			_mm_store_si128((__m128i *)(Y +  0), x0);
			_mm_store_si128((__m128i *)(Y + 16), x1);
			_mm_store_si128((__m128i *)(Y + 32), x2);
			_mm_store_si128((__m128i *)(Y + 48), x3);
		}
	}
}
#pragma warning( pop )
void convert_yuy2_to_nv12_sse2_aligned(void *frame, BYTE *dst_Y, BYTE *dst_C, const int width, const int height, const int pitch) {
	...
	for (y = 0; y < height; y += 2) {
		...
		for (x = 0; x < width; x += 16, p += 32, pw += 32) {
			...
			_mm_store_si128((__m128i *)(Y + x), x0);
			//-----------1行目終了---------------
			...
			_mm_store_si128((__m128i *)(Y + pitch + x), x0);
			//-----------2行目終了---------------

			x1 = _mm_avg_epu8(x1, x3);  //VUVUVUVUVUVUVUVU
			_mm_store_si128((__m128i *)(C + x), x1);
		}
	}
}
	...
	for (y = 0; y < height; y += 4) {
		for (i = 0; i < 2; i++) {
			...
			for (x = 0; x < width; x += 16, p += 32, pw += 32) {
				...
				_mm_store_si128((__m128i *)(Y + (pitch<<1) + x), x0);
				//-----------3+i行目終了---------------
				...
				_mm_store_si128((__m128i *)(C + x), x0);
			}
		}
	}
}
