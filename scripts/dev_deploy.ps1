...
<#
.SYNOPSIS
  QSVEnc をビルドし、AviUtl2 Plugin へ配置して起動する。
...
[CmdletBinding()]
param(
    [ValidateSet("Release", "Debug", "ReleaseStatic", "DebugStatic")]
...
$Sln = Join-Path $RepoRoot "QSVEnc.sln"
...
if ($Platform -eq "x64") {
    $srcAuo2 = Join-Path $RepoRoot "_build\$Platform\$Configuration\QSVEnc.auo2"
} else {
    $srcAuo2 = Join-Path $RepoRoot "_build\$Platform\$Configuration\QSVEnc.auo"
}
...
$dstAuo2 = Join-Path $PluginDir "QSVEnc.auo2"
Write-Host "==> copy"
Write-Host "    $srcAuo2"
Write-Host " -> $dstAuo2"
...
foreach ($name in @("QSVEnc.ini", "QSVEnc.en.ini", "QSVEnc.zh.ini")) {
