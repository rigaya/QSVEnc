#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""QSVEnc 終了バッチ用: 出力ファイル / ログ / aup2 設定のスモークチェック。"""
        ...
        if "QSVEnc options" in line:
            ...
            for fol in lines[idx + 1 :]:
                ...
                if not s:
                    ...
                    continue
                # 次セクションっぽい行で打ち切り
                if re.match(r"^(AAC|raw |QSVEnc \[|L-SMASH|CPU|encoded |converting |Aviutl)", s):
                    break
                if s.startswith("---"):
                    break
                buf.append(s.strip())
...
def compare_cli(expected: str, actual: str) -> list[str]:
    ...
    if not expected:
        return problems
    if not actual:
        problems.append("ログから QSVEnc options を抽出できませんでした")
        return problems
...
def main() -> int:
    ap = argparse.ArgumentParser(description="QSVEnc smoke check (after-bat)")
    ...
    name_keys = cfg.get("plugin_name_contains") or ["QSVEnc"]
    ...
    if not logpath.is_file():
        ...
        if log_cli:
            ...
            ng("ログから QSVEnc options を抽出できませんでした")
        ...
        if aup2 and aup2.is_file():
            try:
                ...
                if conf is None:
                    ng(f"aup2 から QSVEnc 設定を見つけられません: {aup2}")
                else:
