...
if [ -e /etc/lsb-release ]; then
    ...
    if [ "${PACKAGE_OS_CODENAME}" = "focal" ]; then
        PACKAGE_DEPENDS="intel-media-va-driver-non-free,intel-opencl-icd,intel-level-zero-gpu,level-zero"
        PACKAGE_DEPENDS="${PACKAGE_DEPENDS},libmfx1,libva-drm2,libva-x11-2,libva-glx2,libigfxcmrt7"
